...
const TABLE: &[u8] = br#"abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%^&*()_+[];:'\"<>?,./ "#;

const SHIFT: [i64; 5] = [1, 64, 4096, 262144, 16777216];

/// The `#else` branch's multipliers.
const VALUES_NEW: [i64; 8] = [8521739, 5266711, 3254959, 2011673, 1243273, 768389, 474899, 293507];
...
impl Randlib {
    ...
    pub fn gennor(&mut self, av: f64, sd: f64) -> f64 {
        assert!(sd >= 0.0, "SD < 0 in GENNOR - ABORT (value of SD: {sd:16.6E})");
        sd * self.snorm() + av
    }
